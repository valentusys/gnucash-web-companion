...
- Branch: `main`
- Status: pre-alpha / v0.1.7-readonly read-only pre-release published in Phase 171; public status reconciliation completed through Phase 172; Phase 177 disposable write-alpha backup/restore drill passed for synthetic data; v0.2.0-writealpha remains a separate published experimental pre-release

## Current baseline

Completed through Phase 177.
...
- Continue only with the next explicitly requested phase. Controlled writes remain post-MVP/experimental, disabled by default, constrained to `APP_ENV=test` copied/disposable fixtures when explicitly enabled, and not safe for production/private/only-copy books. Phase 177 proved only a synthetic/disposable backup restore drill for the write-alpha create path; do not broaden it into production disaster-recovery evidence.
