...
- Phase 215 is a read-only unavailable-book error contract hardening phase: read-only data route families now require accessible local book storage before constructing GnuCash services and return deterministic path-safe `503` responses for missing/not-configured books, while metadata-only `/books` remains available for recovery diagnostics without raw `uri_or_path` exposure. The web 503 page sends operators back to `/books`, and browser/API dogfood verified a synthetic accessible-but-missing book recovery path at mobile and desktop widths with default `GNUCASH_WRITES_ENABLED=false`, hidden write UI, disabled validate/create/PATCH/DELETE probes returning 403, no screenshots/downloads/CSV artifacts, and no real/private data. No book upload/delete/default-changing/registry-edit UI, write-enabled mode, release/tag/package/image, production/security claim, broad compatibility claim, or real/private-book write safety claim was added.
- Phase 216 is a read-only CSV/export and list parity regression phase: backend tests now pin transaction-note query matching parity between list and CSV export plus account-scoped empty/header-only export metadata, while frontend route/static checks reject browser storage for financial filters and `Number()`/`parseFloat()`/`parseInt()` money-string coercion on transaction/account export UI. Synthetic Docker/Caddy dogfood used only the committed fixture copied to ignored runtime data with dummy local-only credentials and fetched CSV through smoke helpers without saving raw export artifacts. `GNUCASH_WRITES_ENABLED=false` remains default; no import, export queue, cap increase, FX conversion, write behavior, release/tag/package/image, real/private book, app DB, backup, `.env`, screenshot/download/raw CSV artifact, token, key, cert, account name, memo, amount, or private financial data was added.
- Phase 217 is a write-alpha backup-failure and no-mutation drill phase: create, PATCH, and DELETE route-family regression tests now inject backup creation failures with path-like details and prove the routes fail before opening the book for write or mutating the disposable synthetic fixture, record failed audit rows without backup paths, return path-safe API/audit errors, and release the per-book lock. Existing post-backup injected create/PATCH/DELETE failure tests continue to prove backup evidence is retained and the fixture remains unchanged. Default-read-only validate/create/PATCH/DELETE route coverage still returns 403 before book resolution/write-service construction. `GNUCASH_WRITES_ENABLED=false` remains default and write-alpha remains executable only under explicit `APP_ENV=test` plus `GNUCASH_WRITES_ENABLED=true` on copied/disposable synthetic fixtures; no new write endpoint, amount/account PATCH, account/import/scheduled write, release/tag/package/image, real/private/only-copy book, app DB, backup artifact, `.env`, screenshot/export, token, key, cert, raw path, account name, memo, amount, production/security claim, or real/private-book write-safety claim was added.
- Phase 214 is a synthetic local upgrade smoke phase: `scripts/smoke/synthetic-upgrade-smoke.sh` verified upgrade from `v0.2.4-writealpha` runtime state to current `main` using a temporary clone, committed synthetic fixture copy, dummy app metadata DB, dummy local-only credentials, Docker Compose/Caddy, `GNUCASH_WRITES_ENABLED=false`, read-only accounts/transactions/reports/scheduled/audit-summary routes, selected-book recovery, and disabled validate/create/PATCH/DELETE probes returning 403. No write-enabled mode, migration wizard, release/tag, package/image, real/private book, app DB, backup, `.env`, screenshot/export, token, key, cert, raw private path, account name, memo, amount, or private financial data was added.
...
Verification result: docs-only validation passed, including `git diff --check`, changed-file scope check, stale release-state grep, GitHub release view after sync, and `JWT_SECRET=<dummy-local-secret> APP_ADMIN_PASSWORD=<dummy-local-password> docker compose config --quiet`. Full backend/frontend suites were intentionally not run because Phase 105 changed only release/status docs and GitHub release metadata, not product code.
...
Status: complete. Public status/release-support docs synchronized after `v0.2.4-writealpha`; automated guard added to CI/local checks.

Goal: remove stale public roadmap/status drift after the Phase 211 `v0.2.4-writealpha` publication and prevent README/PROJECT_STATUS/CHANGELOG/docs/ROADMAP/release-support docs from diverging on current release posture.

Artifacts:

- `scripts/check_public_status.py` — narrow public status guard for current Phase 211 release baseline, `v0.1.7-readonly`, `v0.2.4-writealpha`, `GNUCASH_WRITES_ENABLED=false`, and conservative no-production/no-security/no-stable-claim wording.
- `apps/api/tests/test_public_status_guard.py` — targeted regression tests proving the guard rejects stale current `Phase 172` / `v0.2.0-writealpha` posture and affirmative production claims, while reading only declared public docs/config files.
- `.github/workflows/ci.yml` — foundation job now runs the public status guard.
- `docs/ROADMAP.md`, `README.md`, `README.ru.md`, `CHANGELOG.md`, and `PROJECT_STATUS.md` — public status synchronized to completed Phase 212 while keeping the current release baseline at Phase 211 / `v0.2.4-writealpha`.
...
Status: complete. Synthetic Docker upgrade smoke from `v0.2.4-writealpha` runtime state to current `main` passed with dummy app metadata DB preservation and default-disabled writes.

Goal: verify a safe local upgrade path using only ignored synthetic runtime copies and dummy local-only credentials, without losing app metadata access, default book access, selected-book recovery, or the GnuCash read-only boundary.

Artifacts:

- `scripts/smoke/synthetic-upgrade-smoke.sh` — new synthetic upgrade helper for previous-ref to current-ref Docker/Caddy smoke.
- `apps/api/tests/test_upgrade_metadata_preservation.py` — targeted regression tests for safe app-metadata preservation and idempotent startup seed behavior.
- `docs/dogfood/phase-214-synthetic-upgrade-smoke.md` — redacted smoke command/result and safety evidence.
- `docs/handoff/phase-214.md` — phase handoff.

Safety result: `GNUCASH_WRITES_ENABLED=false` remained default and was never changed to true. The smoke used only a temporary clone, committed synthetic fixture copy, dummy app metadata, and dummy local-only credentials. No real/private book, real app DB, backup, `.env`, screenshot/export, token, key, cert, raw private path, account name, memo, amount, migration wizard, release/tag, package/image, or private financial data was added.

Verification result: synthetic upgrade smoke passed for login, default/selected book metadata, app metadata DB readability, accounts, transactions, reports, scheduled metadata, write-alpha audit-summary, safe selected-book recovery, Docker Compose config, and disabled validate/create/PATCH/DELETE probes returning 403.

## Phase 218 — Write-alpha audit-summary pagination and operator review UX

Status: complete. Phase commit pushed after verification.

Goal: make the read-only write-alpha audit-summary usable for bounded operator review of large synthetic app-metadata audit tables while preserving redaction and app-metadata-only behavior.

Artifacts:

- `apps/api/app/routers/transactions.py`, `apps/api/app/schemas/gnucash_writes.py`, and `apps/api/tests/test_write_alpha_audit_summary.py` — API pagination metadata and large synthetic redaction/access tests.
- `apps/web/src/routes/books/write-alpha-audit/`, `apps/web/src/lib/api/types.ts`, `apps/web/src/lib/i18n/messages.ts`, and `apps/web/scripts/test-auth-routes.mjs` — URL-only pagination/review UI and static safety guards.
- `docs/dogfood/phase-218-audit-summary-pagination-dogfood.md` — synthetic app-metadata-only dogfood evidence.
- `docs/handoff/phase-218.md` — phase handoff.
- `CHANGELOG.md` and `PROJECT_STATUS.md` — status synchronized.

Dogfood result: targeted backend synthetic app-metadata-only audit-summary tests passed for owner/editor paged review, viewer/unauthorized blocking, large synthetic table bounded pages, adjacent page separation, count/status summaries, and malicious row redaction. Frontend static route checks passed for authenticated active-book loading, URL-only filters/pagination, safe field rendering, mobile-safe controls, no browser storage, and no mutation/export/raw payload UI.
...
Status: complete. Release gate failed safely; phase commit prepared after verification. No tag or GitHub pre-release was published.

Goal: perform the final release gate for cycle 2 and publish conservative `v0.2.5-writealpha` only if Phases 212–220 produced safe, meaningful changes/evidence with intact read-only defaults.

Artifacts:

- `docs/release/v0.2.5-writealpha-notes.md` — conservative no-release notes for the evaluated candidate.
- `docs/release/v0.2.5-writealpha-checklist.md` — release checklist with failed write-alpha evidence gate.
...
- `README.md`, `README.ru.md`, `docs/ROADMAP.md`, `CHANGELOG.md`, `PROJECT_STATUS.md`, `scripts/check_public_status.py`, and `apps/api/tests/test_public_status_guard.py` — public status synchronized to completed Phase 221 while keeping `v0.2.4-writealpha` as the current published write-alpha pre-release.

Gate result: local standard checks and default-read-only safety checks passed, candidate tag/release absence was confirmed, and `GNUCASH_WRITES_ENABLED=false` remained default. The release was blocked because Phase 220 found a bounded write-alpha DELETE backup-count anomaly after successful synthetic/disposable DELETE route execution: successful backup-bearing route-family audit rows exceeded backup file count. Therefore `v0.2.5-writealpha` was not published.
...
Status: complete. `v0.2.5-writealpha` was published as a conservative GitHub pre-release after the final Cycle 3 release gate and exact release/status commit CI.

Goal: inspect current `main`, Phase 222–230 remediation/dogfood evidence, release/status artifacts, GitHub tag/release absence, local backend/frontend/Docker checks, public status guard, sensitive-file hygiene, and exact release-commit CI; publish only if all gates pass.

Artifacts:

- `docs/release/v0.2.5-writealpha-notes.md` — conservative pre-alpha write-alpha release notes with disabled-by-default and `APP_ENV=test` safety boundaries.
...
- `docs/release/v0.2.5-writealpha-publication-evidence.md` — publication evidence for tag/GitHub pre-release.
...
- `README.md`, `README.ru.md`, `CHANGELOG.md`, `docs/ROADMAP.md`, `PROJECT_STATUS.md`, `scripts/check_public_status.py`, and `apps/api/tests/test_public_status_guard.py` — public/status guard synchronization to Phase 231 and `v0.2.5-writealpha` as the current published write-alpha pre-release.
- `docs/handoff/phase-231.md` — phase handoff.

Finding: the Phase 220 no-release blocker is closed in the narrow synthetic/disposable evidence sense. Phase 222 fixed backup artifact collision/overwrite risk, Phases 223–225 proved backup/audit/restore evidence, Phase 226 rechecked the default read-only path, Phase 228 rechecked fresh-clone/upgrade with disabled writes, Phase 229 refreshed drift guards, and Phase 230 produced a green final release-candidate dogfood pack. Phase 231 local checks and exact release/status commit CI passed before publication.
