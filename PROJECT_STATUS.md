...
- Branch: `main`
- Status: pre-alpha / v0.1.7-readonly read-only pre-release published in Phase 171; public status reconciliation completed through Phase 172; Phase 182 published `v0.2.1-writealpha`; Phase 191 completed the cycle-2 release-readiness gate and published `v0.2.2-writealpha` as the current experimental write-alpha pre-release; Phase 192 cleaned up the known non-blocking GitHub Actions Node.js 20 deprecation warnings by moving CI JavaScript actions to Node 24-compatible major versions without changing product behavior or runtime defaults; Phase 193 added a stopped-runtime-only, path-redacted cleanup helper for ignored root-owned runtime artifacts and stale/unreadable locks without changing write routes or defaults; Phase 194 made write-alpha create/PATCH/DELETE smoke evidence resilient to root-owned host-side runtime artifacts by falling back to redacted API-container inspection without rerunning mutating routes; Phase 195 hardened the read-only write-alpha audit summary operator UX with safe action/result/time-window filters and redacted count/status metadata; Phase 196 added redacted first-run/read-only deployment diagnostics to `/health` and `/login` for JWT/admin/default-book/CORS/write-mode triage; Phase 197 refreshed GnuCash Desktop fixture compatibility blocker evidence with a disposable GnuCash 4.13 container probe, exact GUI/manual-safe next step, and phase-197 metadata collector redaction/provenance tests; Phase 198 hardened multi-book read-only registry diagnostics, access-role/status copy, unavailable-book actions, and selected-book safe-link recovery without exposing raw paths or adding registry/write management; Phase 199 completed full default-read-only Docker/Caddy regression dogfood on the committed synthetic fixture at mobile and desktop widths, with API smoke, disabled validate/create/PATCH/DELETE 403 probes, auth-cookie/no-overflow/browser artifact checks, and clean teardown; Phase 200 completed bounded cycle-3 write-alpha disposable CRUD/restore dogfood on fresh ignored synthetic runtime copies with create/PATCH/DELETE evidence, audit/backup/lock checks, DELETE restore proof, final default-false read-only smoke, and clean runtime teardown; Phase 201 completed the cycle-3 release-readiness gate and published `v0.2.3-writealpha` as the current experimental write-alpha pre-release after local release checks, exact release-commit GitHub Actions, tag/release absence checks, rendered `GNUCASH_WRITES_ENABLED=false`, and sensitive tracked-file hygiene; Phase 202 hardened the default read-only first-run health/login diagnostics with redacted safe next actions for placeholder secrets/admin bootstrap, default-book mount/readability, CORS posture, and write-disabled status. Phase 207 hardened the read-only write-alpha audit-summary endpoint/UI with bounded counts/status/time-window metadata and stricter row redaction for malicious payload results, timestamps, transaction IDs, path-like errors, amount/account/memo/description-bearing text, and path-like filters without changing write defaults or write scope.
