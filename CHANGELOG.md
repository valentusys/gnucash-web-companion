...
- Phase 100 — completed a non-publishing synthetic/disposable local Docker install smoke on current `main` using the committed synthetic fixture only, with Docker Compose config validation and read-only API smoke coverage for health, login/auth, books/default book, accounts, transactions, transaction detail, CSV export, reports summary, and disabled validate/create/patch write probes. Also fixed narrow smoke-tooling drift so `scripts/smoke/read-only-api-smoke.py --help` is non-mutating, while preserving env-var defaults and adding transaction-detail/CSV-export checks to the smoke script. No tag/release/package was published, writes remain disabled by default, GitHub #39 remains closed, GitHub #38 remains open/blocked, and no real/private data was committed.
...
- Phase 104 — broadened the existing read-only transaction `query` filter for GitHub #11 so transaction list/count and CSV export now match split memo text as well as transaction descriptions, case-insensitively. Frontend helper copy now says search covers descriptions or split memos, and regression coverage proves memo-only matches, count consistency, CSV export parity, description search preservation, and unmatched-query empty results. No backend write path changed, no tag/release/package was published, writes remain disabled by default, and no real/private data was committed.

- Phase 103 — added read-only transaction date-range preset links for GitHub #11 (`This month`, `Last month`, `Year to date`, and `Clear dates`) on the transactions page. Presets populate the existing `date_from`/`date_to` URL parameters, preserve the other active filters for the list and CSV export, keep custom date inputs visible, and add frontend route checks for preset URL behavior, active-summary visibility, and CSV parity. No backend write path changed, no tag/release/package was published, writes remain disabled by default, and no real/private data was committed.

- Phase 102 — refreshed safe GnuCash compatibility generated/disposable fixture evidence for GitHub #22 without a real Desktop-generated fixture: the metadata collector and generated fixture metadata now record safe runtime provenance (collector/generator version, OS, Python, SQLite, and piecash versions), with regression coverage proving private path/account/transaction values are not serialized by the copied-book collector. `gnucash --version` remains unavailable in this environment, so no broad Desktop-version compatibility claim or tag/release/package was published.

- Phase 99 — recorded the non-publishing `v0.1.1-readonly` pre-publish dry-run and authorization guard in `docs/release/v0.1.1-readonly-prepublish-dry-run.md` with the verdict `Ready for explicit authorized publish`. The artifact confirms release notes/checklist/final-gate inputs, local tag absence, GitHub release absence, GitHub Actions state, GitHub #39/#38 state, and exact would-run publish commands marked `NOT EXECUTED`. No tag/release/package was published, writes remain disabled by default, controlled writes remain post-MVP/experimental, and no real/private data was committed.

- Phase 98 — ran the final `v0.1.1-readonly` release-gate verification and recorded `docs/release/v0.1.1-readonly-final-gate.md` with the verdict `Ready for later authorized publish phase`. Backend tests, frontend checks/build, Docker Compose config validation, GitHub Actions/release-state checks, tag/release absence checks, disabled-write/read-only safety confirmation, and sensitive-data hygiene checks passed. No tag/release was published, GitHub #39 remains closed, GitHub #38 remains open/blocked for copied personal-book dogfood, writes remain disabled by default, and no real/private data was committed.

- Phase 97 — prepared conservative draft `v0.1.1-readonly` release notes and a release-prep checklist after the Phase 95 CSV export fix and Phase 96 synthetic benchmark confirmation. The artifacts separate completed evidence, required Phase 98 release-gate checks, known limitations, and publish steps that are intentionally not authorized yet. No tag/release was published, GitHub #39 remains closed, GitHub #38 remains open/blocked for copied personal-book dogfood, writes remain disabled by default, and no real/private data was committed.
