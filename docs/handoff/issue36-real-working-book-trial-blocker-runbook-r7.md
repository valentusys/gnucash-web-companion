# Issue #36 real working-book trial blocker runbook R7 handoff

Date: 2026-06-05
Task id: `real-working-book-trial-blocker-runbook-r7`
...
Updated `docs/write-alpha/real-working-book-trial-runbook.md` with R7
non-mutating blocker detail:

- prerequisite custody checklist for target, backup, runtime, evidence, and
  decision custody;
- no-trial dry review checklist that uses only packet text and conservative guard
  output;
- rollback communication requirements that keep status blocked or in rollback
  review unless reset and disabled-probe proof are available.
...
Commands run after the runbook update and before this handoff was written:
...
Tracked hygiene check passed (1879 tracked paths inspected).
```

The required verification commands were rerun after writing this handoff before
commit; see the final worker status for the final observed output.
