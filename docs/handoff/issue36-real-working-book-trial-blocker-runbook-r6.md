# Issue #36 real working-book trial blocker runbook R6 handoff

Date: 2026-06-05
Task id: `real-working-book-trial-blocker-runbook-r6`
...
Updated `docs/write-alpha/real-working-book-trial-runbook.md` with R6
non-mutating blocker detail:

- authorization expiry and revalidation rules for future approval packets;
- interruption and resume expectations that keep status blocked unless reset and
  disabled-probe evidence is verified;
- explicit documentation-only wording that does not authorize a real
  working-book trial.
...
Commands run after the runbook update and before this report was written:
...
Tracked hygiene check passed (1875 tracked paths inspected).
```
