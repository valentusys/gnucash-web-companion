# release-no-release-decision-docs-r8 handoff
...
- Updated `docs/release/v0.4-owner-writebeta-readiness-unreleased.md` with
  review-order guidance: release authorization must be read before readiness
  evidence.
- Updated `docs/release/v0.4-owner-writebeta-no-release-decision.md` with a
  dependency-order guard that blocks checklist-style wording around approval.
- Updated `docs/release/owner-writebeta-owner-approval-boundary.md` with a
  status-summary rule requiring no-release wording beside accepted evidence.
- Updated `PROJECT_STATUS.md` latest-handoff navigation and current #36 snapshot
  to point at this r8 clarification.
...
All commands passed in the r8 worker run before commit.
