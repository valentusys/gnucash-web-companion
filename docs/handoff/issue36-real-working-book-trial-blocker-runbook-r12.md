# Issue #36 real working-book trial blocker runbook R12 handoff

Date: 2026-06-06
Task id: `real-working-book-trial-blocker-runbook-r12`
...
Updated `docs/write-alpha/real-working-book-trial-runbook.md` with R12
non-mutating blocker detail:

- a future owner/PM re-approval gate when approved packet details change;
- rollback-review handling that remains blocked unless a new exact packet is
  approved in the same future execution context;
- explicit limits on rollback activity: redacted status and restored-copy evidence
  only, never overwriting the source target or committing raw private evidence;
- safe stop wording for reset/disabled-probe uncertainty and no further mutation.
