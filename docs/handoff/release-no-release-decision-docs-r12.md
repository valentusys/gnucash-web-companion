# release-no-release-decision-docs-r12 handoff
...
- Updated `docs/release/v0.4-owner-writebeta-no-release-decision.md` with a
  generated-task provenance guard: backlog flags such as `generated-safe`,
  `no-release`, and `docs-only` bound maintenance work but do not provide release
  authorization.
- Updated `docs/release/owner-writebeta-owner-approval-boundary.md` to state that
  generated backlog metadata is not an approval packet and cannot name release
  scope or accept public-signal risk.
- Updated `PROJECT_STATUS.md` latest-handoff navigation and current #36 snapshot
  to point at this r12 clarification.
