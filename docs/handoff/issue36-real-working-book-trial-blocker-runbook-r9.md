# Issue #36 real working-book trial blocker runbook R9 handoff

Date: 2026-06-06
Task id: `real-working-book-trial-blocker-runbook-r9`
...
Updated `docs/write-alpha/real-working-book-trial-runbook.md` with R9
non-mutating blocker detail:

- a future approval-to-runtime gate sequence that keeps documentation approval
  separate from runtime arming;
- immediate pre-arming rechecks for default-disabled writes, `APP_ENV=test`,
  target custody, independent backup/restore-to-copy proof, writer exclusion,
  and redacted evidence;
- approval revocation and post-reset blocker rules that return the trial to
  blocked status instead of partial success.
