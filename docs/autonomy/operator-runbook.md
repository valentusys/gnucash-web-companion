...
Dry-run previews render at most one pass through safe generated policy tasks and never invoke a real agent. When `generate-from-policy` is selected, dry-run continues through the safe policy template list until the list is exhausted or the hard budget expires; it does not stop early just because `--min-tasks` or `--min-runtime-hours` is already satisfied. Operators must review the rendered prompt files before live use, especially each generated task's allowed scope, non-goals, verification commands, and stop/continue recommendation.
