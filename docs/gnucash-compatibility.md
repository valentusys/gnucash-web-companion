...
The probe only checks `gnucash` and `gnucash-cli` command availability and bounded `--version` output. It does not open a book, scan private directories, or record executable paths. In this environment both commands were unavailable, so no Desktop-generated fixture or Desktop-version compatibility row is claimed.
