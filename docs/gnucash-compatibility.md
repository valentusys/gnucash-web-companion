...
| Overnight worker 03 Desktop tooling probe/checker hardening | no Desktop-generated SQLite fixture produced | mocked Desktop/CLI command outputs only; no GUI and no book fixture generated | command/version discovery now fails closed for missing commands, unsafe/private-looking successful output, overlong output, and ambiguous version strings; GUI/headless blockers remain blockers, not evidence | no new Desktop-generated schema marker; existing generated fixture markers remain `Gnucash = 3000000`, `Gnucash-Resave = 19920` | safe probe/checker behavior only; outputs are bounded/redacted and tests use synthetic strings/mocked subprocess results | Tested by `tests/test_gnucash_compatibility_metadata.py` and `tests/test_write_alpha_compatibility_check.py`; #22 remains open |
| Overnight worker 04 Desktop fixture candidate acceptance preflight | no Desktop-generated SQLite fixture produced | acceptance gate for future operator-supplied redacted candidate metadata only | no new Desktop version was tested; the gate requires `desktop-generated-synthetic`, a bounded `GnuCash X.Y` marker, SQLite backend, synthetic/disposable scope, synthetic-disposable evidence marker, and default-read-only validation marker before a Desktop candidate can become a tested row | no new Desktop-generated schema marker; unsafe, copied/private, unsupported backend, path-like, account/memo/description-like, or amount-like evidence fails closed | metadata preflight only; `scripts/preflight_desktop_fixture_candidate.py` reads JSON metadata and never opens a GnuCash book | Tested by `tests/test_compatibility_matrix.py`; acceptance gate only, #22 remains open |
| Overnight worker 12 compatibility matrix report renderer/checker | no Desktop-generated SQLite fixture produced | existing synthetic/redacted compatibility matrix rows only | report rendering remains conservative and non-claiming; no new Desktop version was tested | no new Desktop-generated schema marker; existing generated fixture markers remain `Gnucash = 3000000`, `Gnucash-Resave = 19920` | pure matrix helper renders an operator summary and validates that the summary preserves synthetic/disposable tested, manual-blocked, unclaimed-backend, and Desktop candidate gate boundaries while failing closed for broad claims or private-looking evidence | Tested by `tests/test_compatibility_matrix.py`; #22 remains open |
| Issue #22 Desktop fixture resolution attempt, 2026-06-04 | no Desktop-generated SQLite fixture produced | host tooling/env detection plus manual-safe runbook only | `gnucash-cli --version` reported GnuCash 5.14, but there was no `DISPLAY`, `xvfb-run`, `Xvfb`, or `xdotool`; no safe noninteractive create/save SQLite command was found | no new Desktop-generated schema marker; existing generated fixture markers remain `Gnucash = 3000000`, `Gnucash-Resave = 19920` | classifies the blocker as missing isolated GUI/Xvfb/manual-safe Desktop session and documents exact manual creation, metadata, preflight, and read-only validation steps | Blocker documented in `docs/compatibility/desktop-synthetic-fixture-runbook.md`; #22 remains open |
...
- Overnight worker 03 hardened the Desktop tooling probe/checker against mocked unsafe command output. Successful `--version` output that is private-looking, overlong, or ambiguous now fails closed instead of becoming Desktop evidence; the compatibility checker also fails closed when report execution succeeds but the `gnucash-cli --version` evidence is unsafe or ambiguous.
- Overnight worker 04 added a Desktop fixture candidate acceptance preflight gate. It accepts only redacted metadata for an isolated Desktop-generated synthetic/disposable SQLite candidate with explicit `GnuCash X.Y` version evidence, synthetic-disposable scope/evidence marker, and default-read-only validation marker; copied/private scope, unsupported backend, path-like/private-looking fields, account/memo/description-like values, and amount-like values fail closed. This is an acceptance gate only and did not produce a Desktop fixture.
- The 2026-06-04 #22 resolution attempt found host `gnucash`/`gnucash-cli` tooling, including `gnucash-cli` GnuCash 5.14 version output, but no display, `xvfb-run`, `Xvfb`, or `xdotool`; no safe noninteractive create/save SQLite fixture command was found. The blocker is now exactly an isolated disposable GUI/Xvfb/manual-safe Desktop session that creates the synthetic SQLite fixture outside git, followed by redacted metadata, fail-closed preflight, and default-read-only validation.
...
- The 2026-06-04 blocker packet adds an exact manual-safe runbook at [docs/compatibility/desktop-synthetic-fixture-runbook.md](compatibility/desktop-synthetic-fixture-runbook.md). The current host has GnuCash CLI/Desktop commands but lacks GUI/Xvfb automation, so #22 remains open until the external/manual Desktop fixture is actually created and validated.
- Overnight worker 03 hardened Desktop/CLI discovery fail-closed behavior for #22 with mocked outputs only: unsafe/private-looking successful version output, overlong output, and ambiguous version strings are rejected rather than recorded as evidence, and the post-mutation compatibility checker fails closed if `gnucash-cli --version` evidence is unsafe even when the report command succeeds.
- Overnight worker 04 added `scripts/preflight_desktop_fixture_candidate.py` and matrix helper coverage for future Desktop-generated synthetic fixture candidate metadata. The gate reads JSON metadata only, rejects missing Desktop marker/version evidence, unsupported backend, copied/private scope, path/private-looking fields, account/memo/description/amount-like values, and missing default-read-only validation marker, and keeps Desktop candidates blocked/manual unless preflight passes. This did not generate a Desktop fixture; #22 remains open.
...
Before a future Desktop-generated synthetic candidate can be accepted as a tested row, run the fail-closed metadata preflight. This is an acceptance gate only; it does not generate a Desktop fixture and it never opens a GnuCash book:

```bash
python3 scripts/preflight_desktop_fixture_candidate.py \
  /tmp/desktop-fixture-candidate-metadata.json
```

The preflight accepts only synthetic/disposable SQLite metadata with `desktop-generated-synthetic` provenance, bounded `GnuCash X.Y` Desktop version evidence, a synthetic-disposable evidence marker, and a default-read-only validation marker. It rejects copied/private scope, unsupported backends, raw/private-looking fields, path-like values, account/memo/description-like values, and amount-like values. #22 stays open until an actual isolated Desktop-generated synthetic fixture exists and passes this gate plus default-read-only validation.
