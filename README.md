...
- Current published write-alpha pre-release: [`v0.2.5-writealpha`](https://github.com/valentusys/gnucash-web-companion/releases/tag/v0.2.5-writealpha), published after the Phase 231 cycle-3 release gate: [notes](docs/release/v0.2.5-writealpha-notes.md), [checklist](docs/release/v0.2.5-writealpha-checklist.md), [final gate](docs/release/v0.2.5-writealpha-final-gate.md), [publication evidence](docs/release/v0.2.5-writealpha-publication-evidence.md).
- Phase 221 evaluated `v0.2.5-writealpha` and recorded an explicit no-release verdict because Phase 220 found a bounded write-alpha DELETE backup-count anomaly. Phases 222–228 remediated, documented, and smoke-verified that blocker closure as synthetic/disposable backup-audit and default-disabled fresh-clone/upgrade evidence only. Phase 229 refreshed the public status/release-doc drift guard after that remediation, Phase 230 produced a green final release-candidate dogfood pack, Phase 231 published `v0.2.5-writealpha` only after final local gates and exact release/status commit CI passed, and Phase 232 reconciled the public status/changelog wording after publication. Write-alpha remains pre-alpha/experimental, disabled by default, `APP_ENV=test` gated when explicitly enabled, and not safe for real/private or only-copy books.
...
- Recent post-release maintenance:
  ...
  - Phase 183 tightened write-alpha lock recovery evidence and safe operator guidance for stale/root-owned lock states without automatic lock deletion.
  - Phase 184 ran bounded synthetic/disposable PATCH dogfood for the existing metadata/split-memo route.
  ...
  - Phase 187 hardened multi-book read-only access boundaries and selected-book recovery after write-alpha work.
  - Phase 188 improved read-only reporting correctness for mixed-currency, unknown-base, zero-balance, signed-balance, and drilldown edge cases.
  - Phase 189 reran fresh-clone Docker smokes for the current read-only tag, current write-alpha tag, and `main` with default disabled writes.
  - Phase 190 reran combined cycle-2 release-candidate dogfood with default read-only API/browser checks plus separate explicit local-only create/PATCH/DELETE write-alpha evidence.
  - Phase 191 passed the cycle-2 release-readiness gate and published `v0.2.2-writealpha` as an authorized GitHub pre-release after exact release-commit CI succeeded.
  - Phase 192 cleaned up GitHub Actions Node.js 20 action deprecation warnings without changing product behavior or runtime defaults.
  - Phase 193 added stopped-runtime-only cleanup/recovery tooling for ignored root-owned runtime artifacts and stale/unreadable locks.
  - Phase 194 made write-alpha smoke helpers resilient to root-owned host-side artifacts while preserving exactly-once mutation semantics.
  - Phase 195 hardened the read-only write-alpha audit-summary operator UX with safe filters and redacted count/status metadata.
  - Phase 196 added redacted first-run/read-only deployment diagnostics for JWT/admin/default-book/CORS/write-mode triage.
  - Phase 197 refreshed GnuCash Desktop fixture compatibility blocker evidence with disposable tooling probes and redaction/provenance tests.
  - Phase 198 hardened multi-book read-only registry diagnostics and selected-book recovery without exposing raw paths or adding management/write controls.
  ...
  - Phase 213 verified the published `v0.2.4-writealpha` tag from a fresh clone with synthetic fixture data, default-disabled writes, API smoke, disabled validate/create/PATCH/DELETE probes, and mobile/desktop browser dogfood.
  - Phase 214 added and ran a synthetic local upgrade smoke from `v0.2.4-writealpha` runtime state to current `main`, preserving dummy app metadata DB access, default book access, selected-book recovery, read-only routes, audit-summary metadata, and disabled writes.
