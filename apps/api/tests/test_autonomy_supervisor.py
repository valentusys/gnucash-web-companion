...
def test_backup_restore_docs_only_prompt_keeps_non_mutating_boundaries():
    task = supervisor.Task(
        ...
        goal=(
            ...
            "validate wording and default-disabled safety without touching private data."
        ),
        allowed_scope=(
            "docs/**, scripts/check_* guards, apps/api tests for non-mutating "
            "backup/restore documentation or guard behavior"
        ),
        non_goals=(
            "creating backups from private books; restore into real books; dogfood; "
            "release publication; public write beta claims"
        ),
    ...
    assert "GNUCASH_WRITES_ENABLED=false" in prompt
    assert "APP_ENV=test" in prompt
    assert "restore into real books" in prompt
    assert "public write beta claims" in prompt
    assert "Work only on this task's allowed scope." in prompt
...
def test_generated_policy_rejects_mutating_write_alpha_verification_commands(tmp_path):
    unsafe_commands = (
        "python3 scripts/write_alpha_create_delete_chain.py",
        "python3 scripts/smoke/write-alpha-create-smoke.py",
        "python3 scripts/smoke/write-alpha-patch-smoke.py",
        "python3 scripts/smoke/write-alpha-delete-restore-smoke.py",
    )
    for command in unsafe_commands:
        policy = write_policy(
            tmp_path,
            SAMPLE_POLICY.replace("python3 scripts/check_public_status.py", command),
        )
