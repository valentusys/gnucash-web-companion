...
assert.match(apiTypes, /export type AdminBookOptionList[\s\S]*items: AdminBookOption\[\][\s\S]*total_count: number[\s\S]*limit: number[\s\S]*offset: number[\s\S]*has_next: boolean/s, 'book option list DTO must model the bounded paginated /admin/book-access/books response exactly');
...
assert.match(listServer, /parent\(\)[\s\S]*const currentUser = layoutData\.currentUser[\s\S]*if \(!isCurrentUserAdmin\(currentUser\)\)[\s\S]*users,[\s\S]*loadErrorCode: null/s, '/admin/users load must use parent layout /auth/me data and return no admin payload for normal users');
...
assert.match(newServer, /load: PageServerLoad = async \(\{ cookies, parent \}\)[\s\S]*getAuthToken\(cookies\)[\s\S]*parent\(\)[\s\S]*isCurrentUserAdmin\(layoutData\.currentUser\)/s, '/admin/users/new load must use parent layout /auth/me data');
assert.match(newServer, /if \(!\(await requireAdmin\(fetch, token, cookies\)\)\)[\s\S]*admin_required/s, '/admin/users/new action must check live is_admin before create');
...
assert.match(detailServer, /redirectToSessionChanged\(cookies\)/, 'self reset must clear cookies through the shared fixed session-changed redirect helper');
...
assert.doesNotMatch(`${desktopNav}\n${mobileNav}`, /\$app\/state|page\.data/, 'nav admin visibility must not fall back to client page state');
