...
export async function verifyUx(cdp,base,locale,cases,saveScreenshot) {
    ...
    const failures=cases.filter(x=>x.locale===locale&&(x.lang!==locale||x.clippedRow||x.technical||x.english||!x.closedHelp||x.overflow||!x.start||!x.row||x.start.top<0||x.start.top>=x.bottom||x.row.top>=x.bottom));
