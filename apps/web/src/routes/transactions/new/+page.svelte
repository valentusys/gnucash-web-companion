	...
	<section id="failure-ui-drill-matrix" data-failure-drill-set="stale_preview_rejection target_preflight_rejection writes_disabled_rejection backup_failure lock_failure read_back_failure reset_probe_failure safe_recovery_copy" class="mb-4 rounded-2xl p-4 text-sm" role="status" aria-labelledby="failure-ui-drill-title" aria-describedby="failure-ui-drill-summary failure-ui-drill-evidence-shape failure-ui-drill-redaction" style="border: 1px solid #fecaca; background: #fef2f2; color: #7f1d1d;">
		...
		<ul id="failure-ui-drill-list" class="mt-3 grid min-w-0 gap-2 md:grid-cols-2" aria-label="Redacted failure UI drill checklist">
			{#each redactedFailureUiDrills as drill (drill.id)}
				<li class="min-w-0 rounded-xl p-3" data-failure-drill={drill.id} data-failure-drill-status={drill.status} data-failure-stage={drill.failure_stage} data-failure-drill-scope={drill.evidence_scope} data-failure-surface={drill.surface} data-failure-result-state={drill.result_state} data-failure-http-status={drill.http_status} data-failure-mutation-count={drill.mutation_count} data-failure-no-success-claim={String(drill.no_success_claim)} data-failure-create-count-state={drill.result_panel.create_count_state} data-failure-read-back-verification={drill.result_panel.read_back_verification} data-failure-backup-state={drill.result_panel.backup_state} data-failure-audit-state={drill.result_panel.audit_state} data-failure-reset-probe-summary={drill.result_panel.reset_default_disabled_probe_summary} data-failure-disabled-probe-families={drill.result_panel.disabled_probe_families} data-failure-raw-evidence-included={String(drill.result_panel.raw_evidence_included)} style="border: 1px solid #fecaca; background: #fff7f7;">
					<div class="flex min-w-0 items-start justify-between gap-3">
