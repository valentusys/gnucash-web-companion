...
async function adminActor(fetchFn: typeof fetch, token: string): Promise<AdminActor> {
...
export const load: PageServerLoad = async ({ cookies, fetch, params, url }) => {
	...
	const actor = await adminActor(fetch, token);
	...
	try {
		const user = await apiFetch<AdminUserDetail>(fetch, `/admin/users/${userId}`, token);
		let bookOptions: AdminBookOption[] = [];
		try {
			bookOptions = await apiFetch<AdminBookOption[]>(fetch, '/admin/book-access/books?limit=50&offset=0', token);
		} catch (reason) {
...
export const actions: Actions = {
	...
	enableUser: async ({ cookies, fetch, params }) => {
		...
		if (!actor.isAdmin) return fail(403, { adminErrorCode: 'admin_required' satisfies AdminProblemCode });
		const userId = userIdFromParams(params);
		const result = await adminApiMutationFetch<AdminUserDetail>(fetch, token, `/admin/users/${userId}/enable`, 'POST');
		if (!result.ok) return fail(result.status, { adminErrorCode: result.message });
	...
	disableUser: async ({ cookies, fetch, params, request }) => {
		...
		const userId = userIdFromParams(params);
		const result = await adminApiMutationFetch<AdminUserDetail>(fetch, token, `/admin/users/${userId}/disable`, 'POST');
		if (!result.ok) return fail(result.status, { adminErrorCode: result.message });
	...
	revokeAccess: async ({ cookies, fetch, params, request }) => {
		...
		const result = await adminApiMutationFetch<null>(fetch, token, `/admin/users/${userId}/book-access/${bookId}`, 'DELETE');
		if (!result.ok) return fail(result.status, { adminErrorCode: result.message });
