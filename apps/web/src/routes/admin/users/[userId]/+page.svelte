<script lang="ts">
	import type { AdminBookAccessRole, AdminBookOption, AdminProblemCode, AdminUserDetail } from '$lib/api/types';
	...
	function bookName(bookId: number, fallback: string): string {
		return data.bookOptions.find((book) => book.id === bookId)?.name ?? fallback;
	}
</script>
...
<main class="mx-auto max-w-5xl px-4 py-8">
	...
	{:else}
		...
		<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
			<form method="POST" action="?/updateDisplayName" class="min-w-0 rounded-2xl border p-4" style="border-color: var(--app-border); background-color: var(--app-card-bg);">
		...
		<section class="mt-4 rounded-2xl border p-4" style="border-color: var(--app-border); background-color: var(--app-card-bg);">
			...
			<form method="POST" action="?/resetPassword" class="mt-4 grid min-w-0 gap-3 md:grid-cols-2">
				...
				<form method="POST" action="?/grantAccess" class="mt-4 grid min-w-0 gap-3 md:grid-cols-[minmax(0,1fr)_12rem_auto]">
					<label class="text-sm font-medium" style="color: var(--app-text);">
						...
						<select name="book_id" required class="mt-1 min-h-11 w-full min-w-0 rounded-lg border px-3 py-2" style="border-color: var(--app-border); background-color: var(--app-bg); color: var(--app-text);">
							{#each data.bookOptions as book (book.id)}
					...
					{#each data.user.assignments as assignment (assignment.book_id)}
						<article class="min-w-0 rounded-xl border p-3" style="border-color: var(--app-border); background-color: var(--app-bg);">
							<div class="flex min-w-0 flex-col gap-3 md:flex-row md:items-start md:justify-between">
								...
								<form method="POST" action="?/revokeAccess" class="min-w-0 rounded-lg border p-3" style="border-color: var(--app-border);">
