...
FORBIDDEN_PRIVATE_EVIDENCE_MARKERS = (
    ...
    "TRANSACTION_DESCRIPTION:",
    "REAL_TRANSACTION_DESCRIPTION=",
    "REAL_TRANSACTION_DESCRIPTION:",
    "PRIVATE_TRANSACTION_DESCRIPTION=",
    "PRIVATE_TRANSACTION_DESCRIPTION:",
    "REAL_TRANSACTION_MEMO=",
    "REAL_TRANSACTION_MEMO:",
    "PRIVATE_TRANSACTION_MEMO=",
    "PRIVATE_TRANSACTION_MEMO:",
    "TRANSACTION_MEMO=",
    "TRANSACTION_MEMO:",
    "REAL_TRANSACTION_AMOUNT=",
    "REAL_TRANSACTION_AMOUNT:",
    "PRIVATE_TRANSACTION_AMOUNT=",
    "PRIVATE_TRANSACTION_AMOUNT:",
    "TRANSACTION_AMOUNT=",
    "TRANSACTION_AMOUNT:",
)
...
FORBIDDEN_PRIVATE_EVIDENCE_LABEL_PATTERNS = (
    ...
    re.compile(r"^\s*(?:[-*+]\s+|>\s*)?(?:raw[ _-])?evidence[ _-]path\s*[:=]", re.I),
    re.compile(r"^\s*(?:[-*+]\s+|>\s*)?(?:csv[ _-])?export[ _-]path\s*[:=]", re.I),
    re.compile(r"^\s*(?:[-*+]\s+|>\s*)?(?:private[ _-])?screenshot[ _-]path\s*[:=]", re.I),
    re.compile(r"^\s*(?:[-*+]\s+|>\s*)?(?:image|artifact)[ _-]path\s*[:=]", re.I),
    ...
    re.compile(r"^\s*(?:[-*+]\s+|>\s*)?(?:real|private|raw)?[ _-]*transaction[ _-](?:description|memo|amount)\s*[:=]", re.I),
...
FORBIDDEN_UNSAFE_AFFIRMATIVE_PATTERNS = (
    ...
    re.compile(r"\bpublic\s+write[-\s]+beta\s+(?:is\s+)?(?:release[-\s]?ready|ship[-\s]?ready|launch[-\s]?ready|rollout[-\s]?ready)\b", re.I),
    re.compile(r"\bpublic\s+write[-\s]+beta\s+(?:is\s+)?(?:approved|authorized|published|released)\b", re.I),
    re.compile(r"\bpublic\s+write[-\s]+beta\s+has\s+been\s+(?:approved|authorized|published|released)\b", re.I),
    ...
    re.compile(r"\bwrite[-\s]+beta\s+(?:approved|authorized|published|released)\b", re.I),
    re.compile(r"\bwrite[-\s]+beta\s+has\s+been\s+(?:approved|authorized|published|released)\b", re.I),
    ...
    re.compile(r"\bwrite[-\s]+beta\s+(?:is\s+)?(?:ga|general\s+availability|generally\s+available|production[- ]safe|production\s+safe|field[- ]tested)\b", re.I),
    re.compile(r"\bwrite[-\s]+beta\s+(?:ga|general\s+availability|general\s+availability\s+release|generally\s+available)\s+(?:is\s+)?(?:ready|approved|authorized|published|released|available)\b", re.I),
    ...
    re.compile(r"\b(owner[- ]?)?writebeta\s+(?:approved|authorized|published|released)\b", re.I),
    re.compile(r"\b(owner[- ]?)?writebeta\s+has\s+been\s+(?:approved|authorized|published|released)\b", re.I),
    re.compile(r"\b(owner[- ]?)?writebeta\s+(?:is\s+)?(?:ga|general\s+availability|generally\s+available|production[- ]safe|production\s+safe|field[- ]tested)\b", re.I),
    ...
    re.compile(r"\bowner[-\s]+write[-\s]+beta\s+(?:approved|authorized|published|released)\b", re.I),
    re.compile(r"\bowner[-\s]+write[-\s]+beta\s+has\s+been\s+(?:approved|authorized|published|released)\b", re.I),
    ...
    re.compile(r"\bbroad\s+GnuCash\s+(?:Desktop\s+)?compatibility\s+(?:has\s+)?(?:been\s+)?(?:claimed|proven|validated|verified|confirmed)\b", re.I),
    ...
    re.compile(r"\b(?:all|any|every)\s+GnuCash\s+(?:Desktop\s+)?versions?\s+(?:(?:is|are)\s+)?write[- ]compatible\b", re.I),
    ...
    re.compile(r"\b(?:release|software|deployment|build)\s+(?:is|are)\s+(?:stable|production[- ]ready|production\s+ready|security[- ]audited)\b", re.I),
    re.compile(r"\bproduction\s+ready\s+(release|software|deployment|build)\b", re.I),
