...
UNSAFE_AFFIRMATIVE_PATTERNS = [
    ...
    re.compile(r"\bowner[-\s]+write[-\s]+beta\s+(?:is\s+)?(?:ga|general\s+availability|generally\s+available|production[- ]safe|production\s+safe|field[- ]tested)\b", re.I),
    ...
    re.compile(r"\b(?:all|any|every)\s+GnuCash\s+(?:versions|backends)\s+(?:(?:is|are)\s+)?(?:supported|compatible|write[- ]compatible)\b", re.I),
